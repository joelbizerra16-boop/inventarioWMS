...
from inventario.services.contagem import ContagemDuplicadaError, salvar_contagem
...
class PocketServiceTestCase(TestCase):
    ...
    def test_mesmo_produto_mesma_posicao_bloqueado(self):
        salvar_contagem(
            self.inventario,
            self.posicao,
            self.produto,
        ...
        with self.assertRaises(ContagemDuplicadaError):
            salvar_contagem(
                self.inventario,
                self.posicao,
                self.produto,
...
class PocketViewTestCase(ClienteAutenticadoMixin, TestCase):
    ...
    def test_pocket_bloqueia_recontagem_duplicada_mesma_posicao(self):
        InventarioItem.objects.create(
            inventario=self.inventario,
            posicao=self.posicao,
            produto=self.produto,
        ...
        self.assertIn('já inventariado', response.json()['message'].lower())
        item = InventarioItem.objects.get(inventario=self.inventario)
        self.assertEqual(item.quantidade_fisica, Decimal('5'))
    ...
    def test_limpa_posicao_apos_salvar(self):
        self.client.post(
            reverse('pocket:contagem', args=[self.inventario.pk]),
            {
                'codigo_posicao': 'PKV-Z01',
        ...
        self.assertNotContains(response, 'value="PKV-Z01"')
    ...
    def test_rejeita_quantidade_zero(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Quantidade deve ser maior que zero')

    def test_rejeita_quantidade_vazia(self):
        response = self.client.post(
            reverse('pocket:contagem', args=[self.inventario.pk]),
            {
                'codigo_posicao': 'PKV-Z01',
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Quantidade obrigatória')
    ...
    def _ajax_post_contagem(self, dados):
    ...
    def test_contagem_ajax_com_lock_posicao(self):
        lock = self._ajax_post_contagem({
            'acao': 'lock_posicao',
            'codigo_posicao': 'PKV-Z01',
        })
        self.assertEqual(lock.status_code, 200)
        self.assertTrue(lock.json()['ok'])

        save = self._ajax_post_contagem({
            'codigo_posicao': 'PKV-Z01',
            'codigo_produto': 'PKV001',
        ...
        item = InventarioItem.objects.get(inventario=self.inventario)
        self.assertEqual(item.quantidade_fisica, Decimal('50'))

    def test_contagem_duplicada_ajax_bloqueada(self):
        self._ajax_post_contagem({
            'codigo_posicao': 'PKV-Z01',
            'codigo_produto': 'PKV001',
            'quantidade_fisica': '10',
        })
        response = self._ajax_post_contagem({
            'codigo_posicao': 'PKV-Z01',
            'codigo_produto': 'PKV001',
            'quantidade_fisica': '5',
        })
    ...
    def test_inventario_inexistente_retorna_id_na_mensagem(self):
        response = self.client.post(
            reverse('pocket:contagem', args=[99999]),
            {
                'inventario_id': '99999',
                'codigo_posicao': 'PKV-Z01',
                'codigo_produto': 'PKV001',
        ...
        self.assertEqual(response.status_code, 400)
        self.assertIn('#99999', response.json()['message'])


class PocketMestresSyncTestCase(ClienteAutenticadoMixin, TestCase):
    def setUp(self):
        self.user = self.autenticar_cliente()
        self.posicao = Posicao.objects.create(codigo='SYNC01', posicao='Sync 01')
        self.produto = Produto.objects.create(
            codigo_produto='SYNC001',
            descricao='Produto Sync',
            setor='A',
        )

    def test_dados_mestres_retorna_posicoes_produtos(self):
        response = self.client.get(reverse('pocket:dados_mestres'))
        self.assertEqual(response.status_code, 200)
        dados = response.json()
        self.assertTrue(dados['ok'])
        self.assertIn('SYNC01', dados['mapa_posicoes'])
        self.assertIn('SYNC001', dados['mapa_produtos'])

    def test_dados_mestres_inclui_posicao_criada_apos_abertura(self):
        primeira = self.client.get(reverse('pocket:dados_mestres')).json()
        self.assertNotIn('NOVA99', primeira['mapa_posicoes'])

        Posicao.objects.create(codigo='NOVA99', posicao='Nova 99')

        segunda = self.client.get(reverse('pocket:dados_mestres')).json()
        self.assertIn('NOVA99', segunda['mapa_posicoes'])
        self.assertEqual(segunda['mapa_posicoes']['NOVA99'], 'Nova 99')
