...
SESSION_LOTE_SKU_IDS = 'ciclico_lote_sku_ids'
SESSION_LOTE_INFO = 'ciclico_lote_info'
...
def limpar_lote_sessao(session) -> None:
    session.pop(SESSION_LOTE_SKU_IDS, None)
    session.pop(SESSION_LOTE_INFO, None)
...
def obter_lote_sessao(session) -> list[int]:
    return session.get(SESSION_LOTE_SKU_IDS, [])


def obter_lote_execucao_info(session) -> LoteExecucaoInfo | None:
    dados = session.get(SESSION_LOTE_INFO)
    if not dados:
        return None
    return LoteExecucaoInfo(**dados)


def gerar_lote_execucao(session, config: ConfiguracaoExecucao) -> list[CicloInventarioSku]:
    ...
    session[SESSION_LOTE_SKU_IDS] = [sku.pk for sku in lote]
    session[SESSION_LOTE_INFO] = {
...
def _obter_skus_lote_sessao(session) -> list[CicloInventarioSku]:
    ciclo = _obter_ciclo_ativo()
    if ciclo is None:
        return []
    ids = obter_lote_sessao(session)
    if not ids:
        return []
    skus = {
        sku.pk: sku
        for sku in CicloInventarioSku.objects.filter(ciclo=ciclo, pk__in=ids)
    }
...
def _recalcular_consolidacao_sku(
    ...
    *,
    ...
    if deferir_status_parcial_pocket and diferenca == 0:
        sku.status_contagem = StatusItemCiclico.VALIDADO
        sku.save()
        _sincronizar_status_itens_contados_sku(sku)
        _registrar_historico_consolidacao(sku, usuario, fisico_total, agora, tipo_historico)
    ...
    if (
        deferir_status_parcial_pocket
        and not sku_contagem_pocket_completa(sku)
    ):
        sku.status_contagem = (
            StatusItemCiclico.RECONTAGEM
            if recontagem
            else StatusItemCiclico.PENDENTE
        )
        sku.save()
        return
...
def obter_ciclo_atual(session=None) -> CicloInfo | None:
    ...
    lote_execucao = obter_lote_execucao_info(session) if session is not None else None
    return CicloInfo(
        pk=ciclo.pk,
