...
class CiclicoHistoricoTestCase(CiclicoAuditoriaBaseMixin, ClienteAutenticadoMixin, TestCase):
    ...
    def test_lista_historico_exibe_ciclo_encerrado(self):
        ...
        encerrar_ciclo()
        response = self.client.get(reverse('historico_unificado') + '?tipo=CICLICO')
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, f'#{ciclo.pk}')
        self.assertContains(response, 'Finalizado')
        self.assertContains(response, 'Histórico de Inventários e Ciclos')

    def test_detalhe_historico_exibe_indicadores(self):
        ciclo = criar_ciclo(usuario_criacao=self.user)
        encerrar_ciclo()
        response = self.client.get(reverse('historico_detalhe', args=['CICLICO', ciclo.pk]))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Total de Itens')
