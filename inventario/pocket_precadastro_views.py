    ...
    def post(self, request):
        ...
        try:
            criar_precadastro_posicao(
                codigo_completo=form.cleaned_data.get('codigo', ''),
                rua=form.cleaned_data.get('rua', ''),
                predio=form.cleaned_data.get('predio', ''),
                nivel=form.cleaned_data.get('nivel', ''),
                apto=form.cleaned_data.get('apto', ''),
                usuario=_usuario_operacional(request),
                origem='POCKET_OPERADOR',
                equipamento=_equipamento_request(request),
            )
...
class PocketPrecadastroPosicaoView(RequerNaoOperadorMixin, RequerEscritaInventarioMixin, View):
    ...
    def get(self, request, inventario_id):
        ...
        return render(request, self.template_name, {
            'inventario': inventario,
            'form': PrecadastroPosicaoForm(initial={'codigo_completo': codigo}),
        })
    ...
    def post(self, request, inventario_id):
        ...
        try:
            criar_precadastro_posicao(
                codigo_completo=form.cleaned_data.get('codigo_completo', ''),
                posicao_descricao=form.cleaned_data.get('posicao_descricao', ''),
                rua=form.cleaned_data.get('rua', ''),
                predio=form.cleaned_data.get('predio', ''),
                nivel=form.cleaned_data.get('nivel', ''),
                apto=form.cleaned_data.get('apto', ''),
