...
class CiclicoProfissionalTestCase(
    ...
    def test_lote_cinco_skus_compartilhado_entre_usuarios(self):
        from django.test import Client

        from inventario.services.ciclico import obter_lote_execucao_ativo

        for indice in range(10):
            self._sap(f'L{indice}', 'IBC', Decimal('10'))
        self.client.post(reverse('ciclico'), {'acao': 'criar'})
        ...
        self.assertEqual(lote.itens.count(), 5)

        operador_user, _ = criar_usuario_teste(
            username='operador.lote5',
        ...
        self.assertContains(response, 'Lote ativo: 5 SKU(s)')

        supervisor_novo = Client()
        supervisor_novo.force_login(self.user)
        self.assertEqual(len(obter_skus_ciclo(session=None)), 5)
