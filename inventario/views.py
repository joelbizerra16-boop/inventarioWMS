...
from inventario.services.contagem import excluir_contagem, salvar_contagem
    ...
    def post(self, request):
        ...
        try:
            if acao == 'gerar_lote':
                ...
                lote = gerar_lote_execucao(request.session, config)
                messages.success(
                    request,
...
class CiclicoHistoricoView(AcessoOperacionalMixin, View):
    template_name = 'inventario/ciclico_historico.html'

    def get(self, request):
        termo = request.GET.get('q', '').strip()
        status_filtro = request.GET.get('status', '').strip()
        return render(request, self.template_name, {
            'linhas': listar_historico_ciclos(termo=termo, status_filtro=status_filtro),
            'termo_busca': termo,
            'status_filtro': status_filtro,
            'status_opcoes': StatusHistoricoCiclo.FILTROS,
            'pode_administrar': usuario_pode_escrever_cadastros(request.user),
        })


class CiclicoHistoricoDetalheView(AcessoOperacionalMixin, View):
    template_name = 'inventario/ciclico_historico_detalhe.html'
